import random
def generateStockTimeframe(first, last, variance, amount, maxVariance):
	timeframe = [first]
	increments = (last - first) / (amount-2)
	for x in range(amount-2):
		variant = random.randrange(-variance, variance)
	...
	return timeframe
print(generateStockTimeframe(381, 364, 10, 14, 25))